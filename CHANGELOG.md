...
- **First measurement: 0.0% false positives at every threshold across 628 paragraphs, Wilson 95% CI 0.0–0.6%, worst paragraph 11 out of 100.** The corpus is nine public-domain works (1788–1907) plus 34 of the maintainer's own blog posts from 2019 to October 2022, published before ChatGPT and in the register this tool is actually pointed at. At the document-score level the detector does not fire on human prose.
- **The flag level says something else, and `detect` mode shows users flags rather than scores.** On the maintainer's own pre-LLM posts, `em-dash` fires on 17.2% of paragraphs and `tier1` on 11.7%. The Tier 1 words responsible are `embrace` (9), `leverage` and inflections (8), `in order to` (5), `when it comes to` (4), `that said` (4). Those are not AI tells in that text; they are a marketer's ordinary 2019 vocabulary, written years before the models existed. Recorded in `corpus/README.md` as a decision to make rather than a defect to patch.
