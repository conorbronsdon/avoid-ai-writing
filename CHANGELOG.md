...
- **First measurement (303 paragraphs, nine public-domain works, 1788–1907): 0.0% at every threshold, Wilson 95% CI 0.0–1.3%, and no paragraph scored above 8 out of 100.** The honest reading is that the corpus is too easy rather than that the detector is calibrated: nineteenth-century prose is a poor proxy for the LinkedIn posts and READMEs this tool is actually pointed at, and `corpus/README.md` says so at length rather than quoting the zero as a result.
- Two defects surfaced from that run and are filed rather than patched here: `hedge-stack` matches ordinary negation such as "could not possibly" (#69), and the `em-dash` flags on this corpus are largely an artifact of era and Gutenberg transcription, which means nineteenth-century text cannot test that rule at all.
