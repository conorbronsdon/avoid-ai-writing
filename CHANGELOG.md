...
- Two subtractive changes, so the rule cannot begin firing on anything it did not already fire on. `maskCode()` blanks fenced blocks, inline code spans, and indented code blocks before counting — `fenceRanges()` existed but was wired only to `title-case-header`, so a tag quoted in backticks counted as a tag used. `isSocialTag()` subtracts all-digit forms (`#88`), 6- and 8-digit hex colours (`#1a2b3c`), and C preprocessor directives (`#include`). `owner/repo#88`, URL fragments, shebangs, and Markdown headings already passed on the rule's own anchor and are untouched.
...
- **3- and 4-digit hex is deliberately *not* carved out.** An earlier version subtracted it, which silently deleted `#b2b`, `#e2e`, `#dad`, `#cafe`, `#ace` and `#face` — real tags, and losing them cost true positives on exactly the stuffed-block shape the rule exists to catch. Real palettes are dominated by 6-digit values, so a CSS paragraph still lands under the threshold without the short forms. Adversarial review caught this; the first version of the fix traded a false positive for a false negative.
...
- Nine fixtures, six that must not fire and three that must. Every carve-out and every masking pass is mutation-tested: disabling any one of them fails the suite. No new detector `type`, so the catalog count stays 61 and `CATEGORIES.md` is unchanged.
