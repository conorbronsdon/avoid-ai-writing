...
### Emotional flatline
- Treat this as a **style heuristic, not an authorship signal**. A targeted run of this repo's current corpus produced zero `emotional-flatline` hits in both classes: 0/376 human vs. 0/530 machine documents, with the paragraph cross-check also at 0/889 human vs. 0/779 machine. There is therefore no in-repo lift estimate for the rule. Every machine unit in this corpus predates 2025; any detector performance measured on that machine leg is at best an upper bound for current models, and this zero-hit result cannot estimate current-model behavior. StoryScope reports the opposite direction in narrative fiction — machine text more often performs emotion through bodily cues, while human text more often names the feeling outright — but transfer from fiction to blogs, docs, social posts, and technical writing is unproven.
