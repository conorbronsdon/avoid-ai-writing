...
- Treat this as a **style heuristic, not an authorship signal**. The current corpus contains no examples in either class, so it cannot estimate a direction. Under the repository's precision-first evidence policy, an unvalidated style rule stays visible but does not move the authorship score.
