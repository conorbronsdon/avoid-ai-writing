...
| proof point / proof (as a strategy word — ZK, crypto, post-quantum genre) | Literal cryptographic proof stays. Use: evidence, demo, shipped piece, validation signal |
| receipt (as a strategy word — same genre) | Name the thing a reader can inspect: working example, public artifact, data point |

**Genre-scoped domain-term collision.** The two rows above apply only where the term is a resident term of art (ZK, crypto, and post-quantum prose) — there the word carries exact meaning to the exact audience, so the generic-strategy use reads as AI lingo where it hurts most. "Evidence" stays fine everywhere when the sentence means support for a claim. The deterministic engine skips both rows, since genre scoping needs judgment it does not have. The general form — any genre where a strategy buzzword collides with a resident term of art — likely travels, and is held here as a note until a second genre instance shows up. Source: `welttowelt/stop-slop-refined` ([#108](https://github.com/conorbronsdon/avoid-ai-writing/issues/108)).
...
- "Turns into," "becomes real," "it stops being" as summary glue: "the concern turns into panic," "a feature turns into a strategy," "the risk becomes real," "it stops being a pile of notes." A costless transformation verb stands in for the thing that happened — no actor, no mechanism, just X relabeled as Y.
- The moral-adjective entry's ontological-slop note touches one corner of this ("the assumption stops being true"); this is the general shape.
- Fix: name the concrete action, threshold, actor, or consequence. "The risk becomes real" becomes "wallets start moving funds." "The concern turns into panic" becomes "the concern forces panic coordination across desks."
- Carve-out, operative: literal and mechanism-stated transformations stay — "the caterpillar turns into a moth," "water turns into ice at zero," and any sentence that states the threshold in the same breath ("the queue turns into a bottleneck once depth passes 10k"). The tell is the bare relabeling with the mechanism missing. Source: `welttowelt/stop-slop-refined` ([#108](https://github.com/conorbronsdon/avoid-ai-writing/issues/108)).
...
### False agency
- Active-voice sentences that hand human judgment, intent, or responsibility to an inanimate subject: "the decision emerged after the offsite," "the culture shifted toward accountability," "a bet lives or dies on distribution." The subject is present and the voice is active, but the actor is a thing that can't decide — each sentence reads fine until you ask who did it, and the responsibility has no owner.
- Third seat in a set of three: agentless passives hide the subject ("support was added"), moral-adjective category errors put character adjectives on non-agents ("an honest shape"), and this one keeps an active subject that can't own the verb.
- Fix: name the human. "The decision emerged after the offsite" becomes "we decided after the offsite." "The data tells us adoption is early" becomes "the funnel numbers read early to us."
- Carve-outs, both operative. Ordinary literal verbs stay: a report can show a result, a system can return an error, a paper can argue a position. Established collective-actor shorthand stays too: "the market rewards shipping" and analyst-register metonymy where the collective is the recognized actor are ordinary English, not hidden responsibility. Flag only when a specific person or team exercised judgment, intent, or choice and the sentence's subject can't be the one who exercised it. Source: `welttowelt/stop-slop-refined` ([#108](https://github.com/conorbronsdon/avoid-ai-writing/issues/108)), carve-outs widened in review.
...
- The detector catches the sentence-initial imperative surfaces with a capitalized product name: "Enter X.", "Meet X, your new favorite...", "Think X meets Y". Two surfaces stay judgment-only on purpose. "Say hello to X" — "Say hello to Grandma." is ordinary human prose — and the bare "Meet X, your new [role]" form, which is how humans introduce colleagues, pets, and babies ("Meet Sarah, your new account manager"); the detector requires a launch-copy head ("your new favorite", "the new standard"). Known accepted false positives, disclosed here: stage directions in dramatic scripts ("Enter Hamlet.") and column-style narrative ("Enter Rashford."). The engine has no fiction mode, so both calls stay with this carve-out. Source: `welttowelt/stop-slop-refined` ([#108](https://github.com/conorbronsdon/avoid-ai-writing/issues/108)).
...
- Carve-out: a writer whose established voice runs on these props keeps them — the register is a tell for *imposed* casualness, not a ban on playfulness. The detector covers only the mechanical props (stage directions, wink asides, "because of course it does"); verdict closers, label-prefix openers, and the self-QA volley need register judgment and stay skill-only. Source: `welttowelt/stop-slop-refined` ([#108](https://github.com/conorbronsdon/avoid-ai-writing/issues/108)).
...
- Related — **persuasive-authority tropes**: "the real question is," "at its core," "fundamentally," "make no mistake," "the truth is," "this matters because," "here's why that matters." Same move as the calibration phrases above, but they assert depth or stakes instead of feeling: they announce that what follows is important rather than showing it. Cut the trope and lead with the substance. Adapted from `blader/humanizer` P27.
...
- Carve-out: literal simultaneity is ordinary narrative and stays unflagged — "she read while everyone else watched the movie," "others debated the amendment." The detector requires a dismissive verb plus the "was still" dramatization marker ("was still debating"), so wire copy, memoir, and fiction using those verbs literally stay clean. The residue — a literal "was still debating" — is an accepted, disclosed false positive. Source: `welttowelt/stop-slop-refined` ([#108](https://github.com/conorbronsdon/avoid-ai-writing/issues/108)).
...
- The reassurance pair is the concession flavor of the same move: "Not always. Not perfectly." after a bold claim — two clipped concessions staged as honesty, carrying no information about when the claim fails. Fold the concession into a clause that says when it fails ("it breaks on cold starts"), or cut it. Source: `welttowelt/stop-slop-refined` ([#108](https://github.com/conorbronsdon/avoid-ai-writing/issues/108)).
