...
- **What doesn't count.** A `#` in technical prose is usually not a tag. Issue and PR references (`#88`, `#1234`), CSS hex colours (`#fff`, `#1a2b3c`), C preprocessor directives (`#include`), URL fragments, `owner/repo#88`, Markdown headings, and anything inside a code span or fence are all subtracted before the threshold applies. A channel name (`#general`) is the same token as a tag and stays counted, since separating them needs a guess about intent.
