...
### Bold list-label periods
- In bulleted lists where each item leads with a short bold label, LLMs end the label with a period (`**Intros.**`, `**Content distribution.**`, `**Developer GTM.**`) and then run the explanation as a separate sentence. A person writing the same list almost always uses a colon instead: `**Intros:**`, `**Content distribution:**`. The colon reads as "here's what this label means"; the period reads as a sentence that the following clause then contradicts by continuing. Example tell: `- **Intros.** Years of conferences and operator network.` becomes `- **Intros:** years of conferences and operator network.` Fix the period to a colon and lowercase the start of the gloss, or drop the bold label and write the point as a plain sentence. Carve-out: when the bold span is a full sentence on its own (not a label introducing a gloss), the period is correct, so leave it.
