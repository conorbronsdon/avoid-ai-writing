...
| `launch-intro` | Launch-copy introduction | Launch-copy dramatic introductions *(the "Meet X, your new favorite…" and "Think X meets Y" surfaces only — bare "Enter X.", bare "Meet X, your new [role]" and "Say hello to X" stay LLM-judgment; see the entry)* |
| `crowd-contrast` | Dramatized crowd contrast | Dramatized contrast against the crowd *(three branches: dismissive verb in **-ing** + "was still" marker, plus the stereotyped "wrote think-pieces" and "played catch-up" wordings, which need no marker — ordinary simultaneity stays clean; a literal "was still debating" is the accepted, disclosed residue)* |
| `fake-casual-prop` | Fake-casual prop | Fake-casual register *(asterisk stage directions and wink asides only — verdict closers, label-prefix openers, the self-QA volley and "because of course it does" stay LLM-judgment)* |
